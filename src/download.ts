        ...
        .then((info) => {
          ...
          if (logging) {
            const data = table([
              ...
              [info.videoDetails.title, "Starting Download", "0%"],
            ]);
            console.info(data);
          }
          video.on("end", () => {
            if (logging) {
              const data = table([
                ["YTDownload", "Status", "Percent"],
                [info.videoDetails.title, "Download Finished", "100%"],
              ]);
              console.log(`\n${data}`);
            }
            resolve(filePath);
          ...
          video.on("progress", (chunk, downloaded, total) => {
            if (logging) {
              const percent = ((downloaded * 100) / total).toFixed(3);
              clearLine(process.stdout, 0);
              cursorTo(process.stdout, 0);
              process.stdout.write(`[YTDownload] Downloading ${percent}%`);
            }
          });
        })
        .catch((err) =>
